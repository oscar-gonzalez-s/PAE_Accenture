...
def updateScore(upd_phist, gen_tel, telfilepath, infilepath, thistorypath, phistorypath, dataframe, relevance, nweeks):
    ...
    if gen_tel == True:
        ...
        engagement, item, gender = extraxtTelegramStatistics(
            telfilepath, thistorypath)
        # Look for all influencers that wore this item, put them on a list
        # filtrate by gender first
        if gender == "WOMAN":
            df = getFilteredDataframe(dataframe, "WOMAN")
        elif gender == "MAN":
            df = getFilteredDataframe(dataframe, "MAN")
...
def predictorEditor(phistorypath, infilepath, nweeks, relevance):
    ...
    for position in indexes:
        ...
        precedent_percent = float(
            df_phist.iloc[position[0], (position[1]-2)].astype(int))/100
        ...
        for influencer_name in inf:
            confidenceEditor(infilepath, variation *
                             (1-relevance), influencer_name)
